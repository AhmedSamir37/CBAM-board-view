...
function playClick() {
  try {
   ...
   - Scenarios: Base / Conservative / Stress (working)
...
function hasEnough(inputs) {
  // شرط “قرار Board-grade”: لازم EU tons >0 و cp>0 و intensity>0
...
/* ---------------- Actions ---------------- */
function setStatus(msg) { if (statusText) setText(statusText, msg); }
...
(function init() {
  ...
  // لو فيه مدخلات مكتملة، احسب تلقائيًا
  const inputs = readInputs();
